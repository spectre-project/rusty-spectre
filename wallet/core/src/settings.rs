...
pub enum WalletSettings {
    #[describe("Network type (mainnet|testnet-8)")]
    Network,
    #[describe("Server address (default: 127.0.0.1)")]
    Server,
    #[describe("Wallet storage or file name (default 'spectre')")]
    Wallet,
}
...
impl DefaultSettings for WalletSettings {
    async fn defaults() -> Vec<(Self, Value)> {
        vec![(Self::Server, to_value("public").unwrap()), (Self::Wallet, to_value("spectre").unwrap())]
    }
}
...
/// Returns the wallet data storage folder `~/.spectre`.
pub fn application_folder() -> Result<PathBuf> {
    Ok(fs::resolve_path(storage::local::default_storage_folder())?)
}

/// If missing, creates the wallet data storage folder `~/.spectre`.
