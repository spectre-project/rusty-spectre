...
use spectre_consensus::processes::mass::MassCalculator;
    ...
    fn build_txs(&mut self) -> Vec<Transaction> {
        ...
        let txs = self
            ...
            .map(|mutable_tx| {
                ...
                let mass = self
                    .mass_calculator
                    .calc_tx_overall_mass(&signed_tx.as_verifiable(), None, spectre_consensus::processes::mass::Kip9Version::Alpha)
                    .unwrap();
