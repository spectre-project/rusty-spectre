    ...
    pub fn new(encoding: Encoding, url: Option<&str>, resolver: Option<Resolver>, network_id: Option<NetworkId>) -> Result<Inner> {
        ...
        .for_each(|notification_op| {
            ...
            interface.notification(
                notification_op,
                workflow_rpc::client::Notification::new(move |notification: spectre_rpc_core::Notification| {
                    ...
                    Box::pin(async move {
                        ...
                        if notification_sender.receiver_count() > 1 {
                            ...
                            log_warn!("WARNING: Spectre RPC notification is not consumed by user: {:?}", notification);
                        }
                        Ok(())
                    })
                }),
...
/// [`SpectreRpcClient`] allows connection to the Spectre wRPC Server via
...
impl SpectreRpcClient {
    /// Create a new `SpectreRpcClient` with the given Encoding and URL
    pub fn new(
        encoding: Encoding,
        url: Option<&str>,
