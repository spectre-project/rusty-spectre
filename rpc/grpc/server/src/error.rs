...
pub enum GrpcServerError {
    ...
    ClosedHandler(SpectredPayloadOps),
...
impl From<GrpcServerError> for spectre_rpc_core::error::RpcError {
    fn from(err: GrpcServerError) -> Self {
        match err {
            GrpcServerError::RpcApiError(err) => err,
            GrpcServerError::NotificationError(err) => err.into(),
            _ => spectre_rpc_core::error::RpcError::General(err.to_string()),
        }
    }
}

impl From<GrpcServerError> for spectre_notify::error::Error {
    fn from(err: GrpcServerError) -> Self {
        match err {
            GrpcServerError::RpcApiError(err) => spectre_notify::error::Error::General(err.to_string()),
            GrpcServerError::NotificationError(err) => err,
            _ => spectre_notify::error::Error::General(err.to_string()),
        }
    }
}
...
impl<T> From<TrySendError<T>> for GrpcServerError {
    fn from(_: TrySendError<T>) -> Self {
        spectre_notify::error::Error::ChannelSendError.into()
    }
}
