...
use spectre_p2p_lib::{
    ...
    pb::{spectred_message::Payload, RequestTransactionsMessage, TransactionNotFoundMessage},
...
impl RelayTransactionsFlow {
    ...
    pub fn invs_channel_size() -> usize {
        ...
        // Note: in go-spectred we have 10,000 for this channel combined with tx channel.
        4096
    }
            ...
            match res {
                ...
                | Err(MiningManagerError::MempoolError(RuleError::RejectNonStandard(..))) => {
                    ...
                    if self.spam_counter % 100 == 0 {
                        spectre_core::warn!("Peer {} has shared {} spam/non-standard txs ({:?})", self.router, self.spam_counter, res);
                    }
                }
                Err(_) => {}
            }
