...
impl Default for SpectredConfig {
    fn default() -> Self {
        Self {
            mute: false,
            path: None,
...
impl TryFrom<SpectredConfig> for Vec<String> {
    type Error = Error;
    fn try_from(args: SpectredConfig) -> Result<Vec<String>> {
        let mut argv = Vec::new();

        if args.path.is_none() {
            return Err(Error::Custom("no spectred path is specified".to_string()));
        }
...
impl Spectred {
    pub fn new(args: SpectredConfig) -> Self {
        Self {
    ...
    pub fn configure(&self, config: SpectredConfig) -> Result<()> {
    ...
    pub fn start(&self) -> Result<()> {
        ...
        if let Some(process) = process {
            if process.is_running() {
                return Err(Error::Custom("Spectre node is already running.".to_string()));
            }
        }
        ...
        // let options = SpectredOptions::new(path,network)?;
    ...
    pub async fn version(&self) -> Result<String> {
        ...
        if let Some(path) = path {
            ...
            Ok("Spectred binary is not configured. Please use 'node select' command.".to_string())
        }
    }
}

#[async_trait]
pub trait SpectredCtl {
    async fn version(&self) -> Result<String>;
    async fn configure(&self, config: SpectredConfig) -> Result<()>;
