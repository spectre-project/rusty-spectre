...
use spectre_math::int::SignedInteger;
...
impl<T: RelationsStoreReader, U: ReachabilityService> RelationsStoreReader for RelationsStoreInFutureOfRoot<T, U> {
    fn get_parents(&self, hash: Hash) -> Result<BlockHashes, spectre_database::prelude::StoreError> {
    ...
    fn get_children(&self, hash: Hash) -> StoreResult<spectre_database::prelude::ReadLock<BlockHashSet>> {
    ...
    fn counts(&self) -> Result<(usize, usize), spectre_database::prelude::StoreError> {
        unimplemented!()
    }
}
...
impl PruningProofManager {
    ...
    pub(crate) fn build_pruning_point_proof(&self, pp: Hash) -> PruningPointProof {
        ...
        let (_db_lifetime, temp_db) = spectre_database::create_temp_db!(ConnBuilder::default().with_files_limit(10));
