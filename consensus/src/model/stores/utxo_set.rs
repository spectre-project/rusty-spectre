...
pub const UTXO_KEY_SIZE: usize = spectre_hashes::HASH_SIZE + size_of::<TransactionIndexType>();
...
impl AsRef<[u8]> for UtxoKey {
    fn as_ref(&self) -> &[u8] {
        ...
        let rposition = self.0[spectre_hashes::HASH_SIZE..].iter().rposition(|&v| v != 0).unwrap_or(0);
        &self.0[..=spectre_hashes::HASH_SIZE + rposition]
    }
}
...
impl TryFrom<&[u8]> for UtxoKey {
    ...
    fn try_from(slice: &[u8]) -> Result<Self, Self::Error> {
        ...
        if slice.len() < spectre_hashes::HASH_SIZE + 1 {
...
impl From<UtxoKey> for TransactionOutpoint {
    fn from(k: UtxoKey) -> Self {
        let transaction_id = Hash::from_slice(&k.0[..spectre_hashes::HASH_SIZE]);
        let index = TransactionIndexType::from_le_bytes(
            <[u8; size_of::<TransactionIndexType>()]>::try_from(&k.0[spectre_hashes::HASH_SIZE..]).expect("expecting index size"),
        );
