...
impl TestContext {
    ...
    pub async fn build_and_insert_disqualified_chain(&mut self, mut parents: Vec<Hash>, len: usize) -> Hash {
        // The chain will be disqualified since build_header_only_block_with_parents builds utxo-invalid blocks
        for _ in 0..len {
            self.simulated_time += self.consensus.params().prior_target_time_per_block;
            let b = self.build_header_only_block_with_parents(parents, 0, self.simulated_time);
    ...
    pub fn build_header_only_block_with_parents(&self, parents: Vec<Hash>, nonce: u64, timestamp: u64) -> MutableBlock {
